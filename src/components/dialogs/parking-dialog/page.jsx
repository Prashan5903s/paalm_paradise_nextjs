...
const schema = object({
    name: pipe(string(), minLength(1, 'Parking name is required'), maxLength(50, 'Parking name max length is 50'))
})

const ParkingDialog = ({ open, setOpen, title = '', fetchZoneData, selectedZone, typeForm, tableData }) => {
    ...
    const submitData = async (formData) => {
        ...
        try {
            const url = selectedZone
                ? `${API_URL}/company/parking/${selectedZone._id}`
                : `${API_URL}/company/parking`
    ...
    return (
        ...
        >
            ...
            <DialogTitle variant="h4" className="text-center sm:pbs-16 sm:pbe-6 sm:pli-16">
                {selectedZone ? 'Edit Parking' : 'Add Parking'}
            </DialogTitle>
