...
const schema = object({
    name: pipe(string(), minLength(1, 'Tower name is required'), maxLength(50, 'Tower name max length is 50'))
})

const TowerDialog = ({ open, setOpen, title = '', fetchZoneData, selectedZone, typeForm, tableData }) => {
    ...
    const submitData = async (formData) => {
        ...
        if (tableData && tableData.length > 0) {
            ...
            if (selectedZone) {
                const exist = tableData.find(item =>
                    item.name.trim().replace(/\s+/g, ' ').toLowerCase() === formData.name.trim().replace(/\s+/g, ' ').toLowerCase() &&
                ...
                const existsInTable = tableData.some(zoneInTable =>
                    zoneInTable.name.trim().replace(/\s+/g, ' ').toLowerCase() == name.trim().replace(/\s+/g, ' ').toLowerCase()
                );
        ...
        try {
            const url = selectedZone
                ? `${API_URL}/company/tower/${selectedZone._id}`
                : `${API_URL}/company/tower`
    ...
    return (
        ...
        >
            ...
            <DialogTitle variant="h4" className="text-center sm:pbs-16 sm:pbe-6 sm:pli-16">
                {selectedZone ? 'Edit Tower' : 'Add Tower'}
            </DialogTitle>
