...
const schema = object({
    name: pipe(string(), minLength(1, 'Visitor name is required'), maxLength(50, 'Visitor name max length is 50'))
})

const VisitorDialog = ({ open, setOpen, title = '', fetchZoneData, selectedZone, typeForm, tableData }) => {
    ...
    const submitData = async (formData) => {
        ...
        try {
            const url = selectedZone
                ? `${API_URL}/company/visitor-type/${selectedZone._id}`
                : `${API_URL}/company/visitor-type`
    ...
    return (
        ...
        >
            ...
            <DialogTitle variant="h4" className="text-center sm:pbs-16 sm:pbe-6 sm:pli-16">
                {selectedZone ? 'Edit Visitor type' : 'Add Visitor type'}
            </DialogTitle>
